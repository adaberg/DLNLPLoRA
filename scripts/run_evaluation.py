...
def load_checkpoint(checkpoint_path: str, config: Dict, device: str) -> nn.Module:
    ...
    if config.get("training_mode") == "lora":
    ...
    elif config.get("training_mode") == "dora":
        # Support DoRA model loading
        lora_config = config.get("lora") or config.get("lora_config", {})
        model = DoRAGPT2(
            base_model=model,
            rank=lora_config["rank"],
        ...
        print("Loaded DoRA model structure")
    ...
    if "model_state_dict" in checkpoint:
        ...
        print(
            f"Loaded model state from checkpoint (epoch {checkpoint.get('epoch', 'unknown')})"
        )
    else:
...
def setup_test_data(config: Dict, tokenizer):
    ...
    sample_percentage = config.get("sample_percentage") or config.get(
        "dataset", {}
    ).get("sample_percentage", 1.0)
