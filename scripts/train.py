...
def setup_model(config: dict, training_mode: str):
    ...
    if training_mode == "lora":
    ...
    elif training_mode == "dora":
        logger.info("Setting up DoRA model...")
        model = DoRAGPT2(
            base_model=base_model,
            rank=config["lora"]["rank"],
            alpha=config["lora"]["alpha"],
        ...
        logger.info(f"DoRA modules injected: {model.lora_modules}")
    elif training_mode == "full":
        logger.info("Setting up full fine-tuning...")
        model = base_model
        for param in model.parameters():
            param.requires_grad = True
    elif training_mode == "qnone":
        logger.info("Setting up quantized model for evaluation only (no training)...")
        # TODO
        model = base_model
        for param in model.parameters():
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="lora",
        choices=["lora", "full", "none", "dora", "qnone"],
    ...
    parser.add_argument(
        "--save_steps", type=int, default=None, help="Save every N steps"
    )
    parser.add_argument(
        "--eval_steps", type=int, default=None, help="Evaluate every N steps"
    )

    # Logging
    parser.add_argument(
        "--logging_steps", type=int, default=None, help="Log every N steps"
    )
...
def main():
    ...
    experiment_name = args.experiment_name or config.get(
        "experiment_name", "lora_training"
    )
    base_output_dir = args.output_dir or config.get("output_dir", "./results")
    output_dir = os.path.join(
        base_output_dir, f"{experiment_name}_{args.mode}_{timestamp}"
    )
    ...
    training_config = TrainingConfig(
        learning_rate=float(
            args.lr or config.get("training", {}).get("learning_rate", 2e-4)
        ),
        weight_decay=float(
            args.weight_decay or config.get("training", {}).get("weight_decay", 0.01)
        ),
        num_epochs=int(args.epochs or config.get("training", {}).get("num_epochs", 5)),
        batch_size=int(
            args.batch_size or config.get("training", {}).get("batch_size", 8)
        ),
        warmup_steps=int(
            args.warmup_steps or config.get("training", {}).get("warmup_steps", 500)
        ),
        max_grad_norm=float(
            args.max_grad_norm or config.get("training", {}).get("max_grad_norm", 1.0)
        ),
        gradient_accumulation_steps=int(
            args.gradient_accumulation_steps
            or config.get("training", {}).get("gradient_accumulation_steps", 1)
        ),
        label_smoothing=float(config.get("training", {}).get("label_smoothing", 0.1)),
        output_dir=output_dir,
        logging_steps=args.logging_steps
        or config.get("training", {}).get("logging_steps", 100),
