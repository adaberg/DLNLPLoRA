...
def evaluate_model_comprehensive(
    ...
    num_beams = generation_config.get(
        "num_beams", generation_config.get("beam_size", 10)
    )
    ...
    if not hasattr(test_dataset, "get_grouped_data"):
        logger.error(
            "test_dataset must have get_grouped_data() method for proper E2E evaluation"
        )
        return results
    ...
    logger.info(
        f"E2E Evaluation: {num_samples} unique MRs (out of {total_unique_mrs} total)"
    )
    logger.info(
        f"Total test samples: {len(test_dataset)}, Average refs per MR: {len(test_dataset)/total_unique_mrs:.1f}"
    )
    logger.info(
        f"Generation config: num_beams={num_beams}, length_penalty={length_penalty}, no_repeat_ngram_size={no_repeat_ngram_size}, do_sample={do_sample}"
    )
